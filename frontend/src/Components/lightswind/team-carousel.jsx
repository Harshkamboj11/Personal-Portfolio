...
import { cn } from '../lib/utils'; // Your classnames util

export const TeamCarousel = ({
  members,
  title = "Projects",
  titleSize = "2xl",
  titleColor = "#e99b63",    // Accent color
  background = "black",
  cardWidth = 280,
  cardHeight = 380,
  cardRadius = 20,
  ...
  titleClassName,
  infoPosition = "bottom",
  infoTextColor = "#e99b63",
  infoBackground = "rgba(0,0,0,0.9)",
  onMemberChange,
  onCardClick,
  initialIndex = 0,
}) => {
  ...
  const getVariantStyles = (position) => {
    ...
    switch (position) {
      ...
      case 'right-1':
        return { zIndex: 5, opacity: sideCardOpacity, scale: sideCardScale, x: cardWidth * 0.7, filter: grayscaleEffect ? 'grayscale(100%)' : 'grayscale(0%)', pointerEvents: 'auto', transition };
      case 'right-2':
        return { zIndex: 1, opacity: sideCardOpacity * 0.7, scale: sideCardScale * 0.9, x: cardWidth * 1.4, filter: grayscaleEffect ? 'grayscale(100%)' : 'grayscale(0%)', pointerEvents: 'auto', transition };
      case 'left-1':
        return { zIndex: 5, opacity: sideCardOpacity, scale: sideCardScale, x: -cardWidth * 0.7, filter: grayscaleEffect ? 'grayscale(100%)' : 'grayscale(0%)', pointerEvents: 'auto', transition };
      case 'left-2':
        return { zIndex: 1, opacity: sideCardOpacity * 0.7, scale: sideCardScale * 0.9, x: -cardWidth * 1.4, filter: grayscaleEffect ? 'grayscale(100%)' : 'grayscale(0%)', pointerEvents: 'auto', transition };
  ...
  // Touch navigation
  const handleTouchStart = (e) => { if (!touchNavigation) return; setTouchStart(e.targetTouches[0].clientX); };
  const handleTouchMove = (e) => { if (!touchNavigation) return; setTouchEnd(e.targetTouches[0].clientX); };
  ...
  return (
    ...
    >
      {title && (
        <h1
          className={cn(
            "font-black uppercase tracking-tight absolute top-0 font-base left-1/2 transform -translate-x-1/2 pointer-events-none whitespace-nowrap",
              ...
              return (
                <motion.div
                  ...
                  style={{
                    ...
                    border: `2px solid ${titleColor}`, // accent colored border
                  }}
                ...
                >
                  <img src={member.image} alt={member.name} className="w-full p-2 rounded-[20px] h-2/3 object-cover" />
                  {infoPosition === 'overlay' && (
                    ...
                    >
                      ...
                      <p className="text-sm opacity-90" style={{ color: titleColor }}>{member.role}</p>
                    </div>
                  )}
                </motion.div>
              );
      ...
      {infoPosition === 'bottom' && members[currentIndex] && (
        ...
        >
          <h2
            className="text-4xl font-bold mb-3 relative inline-block -mt-4"
          ...
          <p
            className="text-xl font-medium opacity-100 uppercase tracking-wider"
            style={{ color: infoTextColor }}
          >
            {members[currentIndex].role}
          </p>
          {members[currentIndex].bio && (
            <p className="text-base mt-4 max-w-lg text-[#e99b63] mx-auto opacity-70 text-justify">
              {members[currentIndex].bio}
            </p>
          )}
        </motion.div>
